...
#define NTP_STALENESS_INTERVAL  86400 // 1 hour = 3600, 6 hours = 21600, 1 day = 86400, 
...
#define ENCODER_DETENTS_PER_CLICK 2   // Not used in new ISR; keep for future tuning
...
void IRAM_ATTR encoderISR() {
  ...
  if (now - lastEncoderTime < ENCODER_DEBOUNCE_MS) return;
...
bool syncTimeWithNTP() {
  ...
  // 1. Complete Reset of WiFi Stack
  WiFi.disconnect(true, true); // Erase config and turn off
  delay(500);
  WiFi.mode(WIFI_STA);
  
  // 2. C3 Specific: Disable Power Save during connection
  // This prevents the radio from sleeping while trying to handshake
  WiFi.setSleep(false); 
  ...
  while (WiFi.status() != WL_CONNECTED && attempts < 40) {
    ...
    // If we get stuck in Status 6, sometimes a re-call to begin() helps
    if (attempts == 15 && WiFi.status() != WL_CONNECTED) {
      Serial.println("\nStatus 6 detected - Re-sending credentials...");
      WiFi.begin(WIFI_SSID, WIFI_PASSWORD);
    }
    attempts++;
  }
...
bool timeIsSane() {
  return rtc.getEpoch() >= 1767225600;
}
...
void displayOverview() {
  ...
  if (millis() - lastAnimTick > 150) {
    ...
    if (animX > 20) { animX = 20; animDir = -1; }
    lastAnimTick = millis();
  }
...
// Encoder rotation: apply hysteresis using ENCODER_DETENTS_PER_CLICK
  ...
  if (abs(deltaTicks) >= ENCODER_DETENTS_PER_CLICK) {
    ...
    lastProcessedTicks += steps * ENCODER_DETENTS_PER_CLICK; // consume those ticks
    ...
    Serial.printf("[ENC] Ticks: %d, Steps: %+d, Pos: %d\n",
                  ticksSnapshot, steps, encoderPos);
    ...
    switch(currentMode) {
      ...
      case MODE_HISTORY:
        ...
        historyIndex -= delta;
        if (historyIndex < 0) historyIndex = 0;
        if (historyIndex >= (int)flashEntryCount) historyIndex = flashEntryCount - 1;
